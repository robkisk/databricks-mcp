    ...
    while status in ["PENDING", "RUNNING"]:
        ...
        if status == "SUCCEEDED":
            ...
            raise DatabricksAPIError(
                f"Query execution failed: {error_message}", response=status_response
            )
...
async def get_statement_status(statement_id: str) -> Dict[str, Any]:
    ...
    return await make_api_request(
        "GET", f"/api/2.0/sql/statements/{statement_id}", params={}
    )
...
async def cancel_statement(statement_id: str) -> Dict[str, Any]:
    ...
    return await make_api_request(
        "POST", f"/api/2.0/sql/statements/{statement_id}/cancel", data={}
    )
