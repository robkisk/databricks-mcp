...
async def test_pull_repo():
    with patch(
        "databricks_mcp.api.repos.make_api_request",
...
async def test_run_notebook():
    with (
        patch(
            "databricks_mcp.api.jobs.submit_run",
            new=AsyncMock(return_value={"run_id": 99}),
        ) as mock_submit,
        patch(
            "databricks_mcp.api.jobs.await_until_state", new=AsyncMock(return_value={})
        ),
        patch(
            "databricks_mcp.api.jobs.get_run_output",
...
async def test_sync_repo_and_run_notebook_tool():
    ...
    with (
        patch(
            "databricks_mcp.api.repos.pull_repo",
            new=AsyncMock(return_value={"pulled": True}),
        ) as mock_pull,
        patch(
            "databricks_mcp.api.jobs.run_notebook",
            new=AsyncMock(return_value={"run_id": 5}),
        ) as mock_run,
    ):
        res = await server.call_tool(
            "sync_repo_and_run_notebook",
            {"params": {"repo_id": 1, "notebook_path": "/nb"}},
        )
